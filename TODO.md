...
### 4. GitHub REST API 1000-runs hard cap — 🔜 known limitation, workaround отложен
- **Симптом:** все активные репо (twirapp/twir, gin-gonic/gin) показывают ровно `runs_count = 1000`, хотя по факту у них десятки тысяч CI-запусков. Видно глазом: цифра подозрительно одинаковая.
- **Причина:** GitHub REST API `/repos/{owner}/{repo}/actions/runs` возвращает максимум 1000 рунов (10 страниц × 100) независимо от `total_count` в ответе. Это документированный лимит, обход требует разбиения запроса на меньшие time-окна через `created=YYYY-MM-DD..YYYY-MM-DD`.
- **Текущий код:** `services/api-gateway/internal/github/sync.go` делает single `created>=cutoff` фильтр + пагинацию до 100 элементов или конца — упирается в 1000.
- **Workaround при реализации:**
  - Разбить `since`-окно на месячные подокна
  - Внутри окна обычная пагинация
  - Если окно даёт ровно 1000 — рекурсивно разделить пополам (super-busy month)
  - Чекпоинт `last_synced_window` вместо просто `last_synced_at`
- **Цена:** ~120 req на большой репо (12 месячных окон × ~10 страниц) при PAT 5000/h — 2 минуты на синк.
- **Размер:** M (1-2 часа на код + тесты).
- **Почему не сделал в этой итерации:** «сделать позже» по решению автора 2026-05-23. 6000 jobs суммарно из 1000 рунов уже достаточно для всех ML-экспериментов; полную историю надо собирать только если захочется демонстрировать «модель обучилась на 50K samples».
- **Для диссертации:** упомянуть в разделе «GitHub API constraints» (Глава 6) как известное ограничение и описанный workaround.
